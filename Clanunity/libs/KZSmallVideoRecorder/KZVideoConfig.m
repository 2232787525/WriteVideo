...
+ (void)motionBlurView:(UIView *)superView {
    superView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.3];
...
+ (instancetype)modelWithPath:(NSString *)videoPath mp4path:(NSString*)mp4Path thumPath:(NSString *)thumPath recordTime:(NSDate *)recordTime {
    KZVideoModel *model = [[KZVideoModel alloc] init];
    model.videoAbsolutePath = videoPath;
    model.videoMP4Path = mp4Path;
...
+(void)deleteCacheVideo{
    NSString *videoPath = [KZVideoUtil getVideoPath];
    NSFileManager * fileManger = [NSFileManager defaultManager];
    BOOL isDir = NO;
    BOOL isExist = [fileManger fileExistsAtPath:videoPath isDirectory:&isDir];
    if (isExist) {
        // 2. 判断是不是目录
        if (isDir) {
            NSArray * dirArray = [fileManger contentsOfDirectoryAtPath:videoPath error:nil];
            NSEnumerator *e = [dirArray objectEnumerator];
            NSString *filename;
            while (filename = [e nextObject]) {
                NSString*videoChildPath =[videoPath stringByAppendingPathComponent:filename];
               BOOL delete = [fileManger removeItemAtPath:videoChildPath error:nil];
                if (delete) {
                    NSLog(@"删除成功 %@",filename);
                }
            }
        }else{
            NSLog(@"不是文件夹%@",videoPath);
        }
    }else{
        NSLog(@"你打印的是目录或者不存在");
    }
...
+ (NSMutableArray *)getVideoList {
    ...
    for (NSString *name in nameList) {
        if ([name hasSuffix:@".JPG"]) {
            ...
            NSString *totalVideoMp4Path = [thumAbsolutePath stringByReplacingOccurrencesOfString:@"JPG" withString:@"MP4"];
            if ([fileManager fileExistsAtPath:totalVideoPath]) {
                model.videoAbsolutePath = totalVideoPath;
            }
            if ([fileManager fileExistsAtPath:totalVideoMp4Path]) {
                model.videoMP4Path = totalVideoMp4Path;
            }
...
+ (KZVideoModel *)createNewVideo {
    ...
    model.videoMP4Path =[videoPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.MP4",videoName]];
...
+ (void)deleteMP4Video:(NSString *)videoPath{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    [fileManager removeItemAtPath:videoPath error:&error];
    if (error) {
        NSLog(@"删除Mp4视频失败:%@",error);
    }
    NSString *thumPath = [videoPath stringByReplacingOccurrencesOfString:@"MOV" withString:@"JPG"];
    NSError *error2 = nil;
    [fileManager removeItemAtPath:thumPath error:&error2];
    if (error2) {
        NSLog(@"删除mp4缩略图失败:%@",error);
    }
}
