...
- (NSString *)dbZFYPathByString:(NSString *)sqlLibraryName{
    ...
    pathString = [pathString stringByAppendingPathComponent:@"ZFY"];//用公司名比较好
    BOOL isDir;
    ...
    if (sqlLibraryName == nil || sqlLibraryName.length == 0) {
        dbpath = [pathString stringByAppendingPathComponent:@"zfy.sqlite"];
    } else {
...
/**
 ...
 @param modelClass 类
 @param saveNameArray 原保存的表的所有字段名称
 ...
 * tabname为空的话表名字就是model的class字符串
 */
-(void)addModelToFmdbTabName:(NSString *)tabname WithModel:(NSObject<JCF_ModelProtocol> *)model AndResult:(JCF_SQLAddModelBlock)block{
...
- (NSString *)searchModelToFmdbTabName:(NSString*)tabname WithModelClass:(Class)modelClass AndInfoDictionary:(NSDictionary *)infoDictionary{
    ...
    //条件查询
    NSMutableString *newSql = [NSMutableString string];
    [newSql appendString:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE",tabname]];
    for (NSInteger i = 0; i < proNamesArray.count;i++ ) {
        NSString *proname = proNamesArray[i];
        id provalue = infoDictionary[proname];
        if ([[provalue class] isSubclassOfClass:[NSString class]]) {
            [newSql appendFormat:@" %@='%@'", proname,provalue];
        }else{
            [newSql appendFormat:@" %@=%@", proname,provalue];
        }
        if(i+1 != proNamesArray.count)
        {
            [newSql appendString:@" AND"];
        }
    }
