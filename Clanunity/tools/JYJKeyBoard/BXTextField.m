...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    NSLog(@"displayingKeyboard ----  %zd", self.displayingKeyboard);
    [self.doneButton removeFromSuperview];
    self.doneButton = nil;
    ...
    if ([UIScreen mainScreen].bounds.size.width == 320) {
        ...
        if( [PLHelp kIphoneType]==kIphoneX){ //iphoneX的宽是375
            doneButtonH = (kbHeight - 2 - KBottomStatusH) / 5;//iphoneX的宽是375
            doneButtonW = ([UIScreen mainScreen].bounds.size.width - 8) / 3 - 5;
            doneButtonX = 5;
    ...
    if (!self.displayingKeyboard) {
        ...
        [UIView setAnimationDuration:animationDuration];
        [UIView setAnimationCurve:animationCurve];
        if ( [PLHelp kIphoneType]==kIphoneX) {
            doneButton.transform = CGAffineTransformTranslate(doneButton.transform, 0, -kbHeight - KBottomStatusH - doneButtonH +10 );
        }else{
