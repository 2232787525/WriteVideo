...
class EmptySwiftView: UIView {
    ...
    private init(frame: CGRect,describe:String?,picName:String?) {
        ...
        if self.describe != nil {
            let desLb = UILabel.init(frame: CGRect.init(x: 0, y: 0, width: KScreenWidth*0.8, height: 100), text: self.describe, pLfont: UIFont.plFont15(), textPLColor: UIColor.plColor12B06B_Theme(), andTextAlignment: NSTextAlignment.center)
            ...
            desLb.height_sd = h
            desLb.top_sd = imgView.bottom_sd + 15
            self.addSubview(desLb)
            self.height_sd = (desLb.bottom_sd)
