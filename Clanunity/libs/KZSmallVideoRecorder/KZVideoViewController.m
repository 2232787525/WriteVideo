...
- (void)startAnimationWithType:(KZVideoViewShowType)showType withSuperView:(UIView*)view{
    _showType = showType;
...
- (void)setupSubViews {
    ...
    CGFloat buttomHeight = kzControViewHeight;
    _topSlideView.backgroundColor = [UIColor greenColor];
    ...
    _videoView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, videoViewSize.width,self.actionView.frame.size.height)];
...
- (void)setupVideo {
    ...
    if ([_videoSession canSetSessionPreset:AVCaptureSessionPreset1920x1080]) {
        _videoSession.sessionPreset = AVCaptureSessionPreset1920x1080;
    }
