...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    NSLog(@"拿到mov格式的视频");
    // 转换
    [self saveThumImageWithVideoURL:outputFileURL second:1 videoThumPathClicked:^(NSString *thumPath) {
        self.thumPath = thumPath;
    }];
...
- (void)saveThumImageWithVideoURL:(NSURL *)videoUrl second:(int64_t)second videoThumPathClicked:(void (^)(NSString *thumPath))block{
    ...
    NSURL *jpgUrl = [videoUrl copy];
    jpgUrl = [jpgUrl URLByDeletingPathExtension];
    jpgUrl = [jpgUrl URLByAppendingPathExtension:@"JPG"];
    
    BOOL isok = [imgData writeToFile:jpgUrl.absoluteString atomically: YES];
    NSLog(@"缩略图获取结果:%d",isok);
    CGImageRelease(cgimage);
    if (isok) {
        block(jpgUrl.absoluteString);
    }else{
        block(nil);
    }
