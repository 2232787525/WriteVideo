...
@property(nonatomic,copy)NSString * videoThumImg;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        {
            //容器
            _sizeContainer = CGSizeMake(KScreenWidth, 150);
            self.frame     = CGRectMake(0, KScreenHeight-_sizeContainer.height, _sizeContainer.width, _sizeContainer.height);
            
            //拍照按钮
            _sizeBtnShoot = CGSizeMake(80, 80);
...
- (void)onChoose:(UIButton *)sender{
    ...
    if (_chooseBlock) {
        switch (_shootType) {
            ...
            {
                _chooseBlock(ShootType_Photo,self.stillImageView.image,nil);
            }
                break;
            case ShootType_Video:{
                _chooseBlock(ShootType_Video,self.videoPath,self.videoThumImg);
            }
                break;
            default:
                break;
...
- (void)_stopShooting{
    ...
    [[YHVideoHelper shareInstanced] stopRecordingVideo:^(NSString *path) {
        ...
        weakSelf.player = [[YHAVPlayer alloc] initWithFrame:weakSelf.superView.frame playerURL:[NSURL fileURLWithPath:path]];
