...
- (void)configScrollViewWith:(NSInteger)clickTag andAppendArray:(NSArray *)appendArray ImageType:(ImageURlAlterRule)rule{
    ...
    for (int i = 0; i < appendArray.count; i ++) {
        ...
        if ([appendArray[i] isKindOfClass:[NSString class]]) {
            ...
            NSURL *urlStr = nil;
            if (rule == CarCirclImageType) {
                urlStr = [NSURL URLWithString:imgStr];
                NSRange range1 = [imgStr rangeOfString:@"_cut"];
                NSRange range2 = [imgStr rangeOfString:@"_small"];
                if (range1.location != NSNotFound) {
                    urlStr = [NSURL URLWithString:[imgStr  stringByReplacingOccurrencesOfString:@"_cut" withString:@""]];
                }else if (range2.location != NSNotFound){
                    urlStr =[NSURL URLWithString:[imgStr  stringByReplacingOccurrencesOfString:@"_small" withString:@""]];
                
                }
                
            }else if(rule == HeadImageImageType){
                urlStr = [NSURL URLWithString:[imgStr  stringByReplacingOccurrencesOfString:@".jpg" withString:@"_big.jpg"]];
            }
            __block SDWaitingView *pv;
            __block UIImageView *waitImage;
            
            [imageView sd_setImageWithPreviousCachedImageWithURL:urlStr andPlaceholderImage:nil options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (!pv) {
                        ...
                        [waitImage sd_setImageWithURL:[NSURL URLWithString:imgStr]];
                ...
                if (image) {
                    ...
                    UIImage *resurtImage = [ UIImage  resoulImage: tempImg andConstWidth:self.frame.size.width];
                    imageView.image      = resurtImage;
                    pv.hidden = YES;
